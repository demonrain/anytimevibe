...
export function registerAdminRoutes(app: FastifyInstance, ctx: AdminContext, sessionCookie: string): void {
  ...
  app.get("/api/admin/users/:userId", async (request, reply) => {
    ...
    const hosts = await sql<Array<Record<string, unknown>>>`
      SELECT id, name, platform, codex_version, claude_version, grok_version, cursor_version, antigravity_version, pi_version, agent_version, created_at, last_seen_at, revoked_at
  ...
  app.get("/api/admin/hosts", async (request, reply) => {
    ...
    const hosts = await sql<Array<Record<string, unknown>>>`
      SELECT
        h.id, h.name, h.platform, h.codex_version, h.claude_version, h.grok_version, h.cursor_version, h.antigravity_version, h.pi_version, h.agent_version,
      ...
      WHERE (${pattern}::text IS NULL OR h.name ILIKE ${pattern} OR u.username ILIKE ${pattern}
        ...
        OR COALESCE(h.antigravity_version, '') ILIKE ${pattern}
        OR COALESCE(h.pi_version, '') ILIKE ${pattern})
        AND (
          ${query.status} = 'all'
