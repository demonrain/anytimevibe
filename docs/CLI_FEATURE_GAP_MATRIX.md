...
当前 `turn.steer` 只在 Codex 分支调用 `turn/steer`，并携带 `expectedTurnId`。这是“向正在运行的同一回合追加方向”，与发送一个新的 `turn.start` 不同，也与停止后重新执行不同。
...
- **抢占当前回合（后续）**：先 interrupt 当前 CLI，再执行新指令。需要保存中断原因、部分输出和恢复点；对非 Codex 引擎不是原生能力，失败时容易造成重复执行或上下文丢失。

首版建议只做队头插入，并允许用户拖动等待项排序；不要默认打断正在运行的模型。
...
| 优先级 | 功能 | 难度 | 建议 |
| --- | --- | --- | --- |
| P0 | Codex 原生 steering 的 Web 入口与状态提示 | 低 | 复用现有 `turn.steer`；活动回合才显示，失败后回退到普通排队 |
...
| P3 | 抢占当前回合 | 高 | 先完成中断恢复、部分输出持久化和 workspace 锁，再考虑开放 |
