...
> 2026-07-11 更新：产品已支持开放注册、多用户数据隔离、客户端环境检测和 macOS Agent。本文后续仍保留部分 MVP 历史描述，容量与公开服务边界以 [多用户容量评估](CAPACITY.md) 为准。
...
AnytimeVibe 是一个面向个人开发者的远程 Codex 工作台。用户通过手机或桌面浏览器中的 PWA，连接自己保持在线的 Windows 电脑，在离开电脑后继续创建、查看和控制 Codex 编程任务。

产品不提供传统远程桌面，也不把项目源码复制到云端。Windows 代理在用户电脑上调用本机 Codex CLI，VPS 中继只负责身份验证、在线路由、Web Push 和端到端加密事件的存储。

### 1.1 产品定位

- 为个人开发者提供“随时继续 Codex 任务”的移动入口。
- 将 Codex 的线程、流式回复、Diff 和审批转换为适合移动端操作的界面。
...
- Windows 代理：Electron 36，x64 NSIS 安装包，当前未进行代码签名。
