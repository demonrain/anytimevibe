...
管理员登录后，右上角头像菜单可进入 **管理后台**（`/admin`），用于用户/主机治理、注册策略与审计。详见 [管理后台说明](ADMIN.md)。
...
同一账号可以在多个手机、平板和浏览器中同时使用同一台远程主机。新浏览器首次访问主机时：

1. 在远程电脑打开随码，点击“生成配对码”。
2. 在新浏览器登录同一随码账号。
3. 点击主机列表旁的 `+`，输入六位配对码并确认。
4. 新浏览器会显示等待电脑授权，Agent 将现有主机同步密钥通过一次性加密通道发送给新浏览器。
5. 授权完成后，新浏览器自动同步任务记录。

该流程不会更换主机同步密钥，已授权的旧浏览器不会失效。中继服务只能看到加密后的密钥包，不能解密主机任务内容。若 Agent 已归属其他用户账号，中继会拒绝授权。
...
发布 Tag 后，GitHub Actions 会在 Windows / macOS runner 上构建客户端并上传到 Release。若暂不提供 macOS 包，可将 `MAC_CLIENT_URL` 留空，Web 端会显示「macOS · 敬请期待」。生产分发应配置 Windows 代码签名以及 Apple Developer ID 签名、公证。
...
Web 创建线程时不会覆盖 Codex 的审批策略和沙箱权限。Agent 启动的 app-server 会读取本机 `~/.codex/config.toml`，因此本机配置为 full access 时，Web 下发的新任务也使用相同权限。工作区白名单仍由随码单独检查，Web 不能在未授权目录创建任务。
...
任务运行时发送的新要求会进入等待队列。对 Codex 任务，可在队列项上点击“插队运行”：客户端会先安全中断当前回合，执行该队列项，再在同一线程中恢复原任务，最后继续处理剩余队列。插队期间不要重复发送同一要求；如果恢复失败，任务会保留错误信息，原有文件修改不会自动撤销。
...
1. 登录 PWA。
2. 电脑浏览器点击顶栏“开启通知”；移动端点击右上角头像，再点击“开启通知”。
3. 允许浏览器发送通知。
4. 收到“随码通知已开启”测试消息后，页面按钮会显示“通知已开启”。

登录时，页面会自动检查并重新登记已经授权的浏览器订阅；服务端 VAPID 公钥变化或浏览器更新订阅后，无需手动清除站点数据。
...
如果按钮提示服务端未配置 Push，请让服务管理员配置 VAPID 密钥并重启 relay 服务。如果没有收到测试消息，请同时检查系统级通知设置、浏览器站点权限以及手机是否允许该 PWA 在后台发送通知。
...
```text
C67162F4688031EC706F4FB47B150D08F359885322252C8F93514625801DC76B
```
