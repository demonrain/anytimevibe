...
# 未发布 macOS 安装包时请留空；Web 端会显示「macOS · 敬请期待」
MAC_CLIENT_URL=
...
发布 Tag 后，GitHub Actions 可在 `macos-latest` 上构建客户端。当前公开 Release 以 Windows 为主；`MAC_CLIENT_URL` 留空时前端会提示「敬请期待」。生产分发应配置 Windows 代码签名以及 Apple Developer ID 签名、公证。
