...
1. 在远程电脑打开 AnytimeVibe Agent，点击“生成配对码”。
2. 在新浏览器登录同一 AnytimeVibe 账号。
...
Web 创建线程时不会覆盖 Codex 的审批策略和沙箱权限。Agent 启动的 app-server 会读取本机 `~/.codex/config.toml`，因此本机配置为 full access 时，Web 下发的新任务也使用相同权限。工作区白名单仍由 AnytimeVibe Agent 单独检查，Web 不能在未授权目录创建任务。
